...
from .models import CNNPolicy, MLPCritic, MonoRacePolicy

# Task IDs that use the MonoRace compact-state perception pipeline
MONORACE_TASKS = {
    "Isaac-Drone-Racer-MonoRace-v0",
    "Isaac-Drone-Racer-MonoRace-Play-v0",
}

# All tasks that require --enable_cameras (have TiledCamera in scene)
CAM_TASKS = {
...
class MonoRaceRunner(Runner):
    """Runner for MonoRace: MonoRacePolicy (compact MLP) + MLPCritic (20-dim ground-truth)."""

    def _generate_models(self, env, cfg):
        device = env.device
        policy_cfg = cfg.get("models", {}).get("policy", {})

        policy = MonoRacePolicy(
            observation_space=env.observation_space,
            action_space=env.action_space,
            device=device,
            clip_actions=policy_cfg.get("clip_actions", False),
