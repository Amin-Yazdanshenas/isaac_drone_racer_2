...
def clip_grad_agc_(parameters: _tensor_or_tensors, clip: float, pmin: float,
    ...
    for (device, _), ([device_params, device_grads], _) in grouped.items():
        if (foreach is None and _has_foreach_support(device_grads, device)) or (
            ...
            scale = torch._foreach_reciprocal(
                torch._foreach_maximum(torch._foreach_div(gnorm, upper), 1.0)
            )
